...
import { toDate } from '@angular/common';
import { text } from '@angular/core/src/render3';
import { locale } from 'moment';
// import { toDate } from '@angular/common/src/i18n/format_date';
// import { DateUtils, DateUtilsToken, IDateUtils, DateUtilsService } from 'axiom-common-services';
...
export class GsoDate implements IGsoDate {
  ...
  constructor(
    ...
    this._standardValue = this.convertToStandardDate(this._initialValue);
    this._type = this.determineType(this._initialValue);
      if (this.config) {
  ...
  toISOString(options?: GsoDateConfig): string {
    ...
    return this._standardValue.toISOString();
  }
  ...
  private determineType(value): GsoDateTypes {
    if (isNgbDateStruct(value)) return GsoDateTypes.NgbDateStruct;
    if (isDate(value)) {
  ...
  private convertToStandardDate(value: any) {
    if (isNgbDateStruct(value)) {
      return this.convertFromNgbDateStruct(value);
    }

    const dateObject = toDate(value);
    return dateObject ? dateObject : this.convertStringWithFormat(value)
  }

  private convertFromNgbDateStruct(value: NgbDateStruct): Date {
    return new Date(value.year, value.month - 1, value.day);
  }

  private convertStringWithFormat(value: any) {
    const namedFormat = getNamedFormat('en-US', this._format);
    this._format = namedFormat || this._format;

    let parts: string[] = [];
    let match;
    while (this._format) {
      match = DATE_FORMATS_SPLIT.exec(this._format);
      if (match) {
    ...
    for (let i = 0; i < parts.length; i += 2) {
      ...
      if (dateFormatter) {
        const part = dateFormatter(value as any, 'en-US', dateTimezoneOffset);
...
function isNgbDateStruct(possibleDate: object): possibleDate is NgbDateStruct {
